...
const FilterChip = ({
  ...
  }, [countryFilter, countryName, defaultButtonStyle]);
  ...
  const onClick = (event, k) => {
    ...
    const newTheme =
      event.target.style.backgroundColor === "white"
        ? { ...buttonStyle, color: "#ffffff", backgroundColor: "#222222" }
        : { ...buttonStyle, color: "#000000", backgroundColor: "white" };
    setButtonStyle({ ...buttonStyle, ...newTheme });
  };

  return (
    <button style={buttonStyle} onClick={(e) => onClick(e, countryName)}>
      {countryName} {numberOfItems}
    </button>
  );
};
